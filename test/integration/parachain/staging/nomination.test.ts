import { BTCAmount, InterBTC, Polkadot } from "@interlay/monetary-js";
...
describe("NominationAPI", () => {
    ...
    it("Should nominate to and withdraw from a vault", async () => {
        ...
        const stakingCapacityBeforeNomination = (await vaultsAPI.getStakingCapacity(newAccountId(api, charlie_stash.address), InterBTC));
        // Deposit
        await nominationAPI.depositCollateral(charlie_stash.address, nominatorDeposit);
        const stakingCapacityAfterNomination = await vaultsAPI.getStakingCapacity(newAccountId(api, charlie_stash.address), InterBTC);
        assert.equal(
        ...
        await issueSingle(api, electrsAPI, bitcoinCoreClient, bob, interBtcToIssue, Polkadot, charlie_stash.address);
        ...
        const nominatorCollateral = await nominationAPI.getTotalNomination(InterBTC, Polkadot, bob.address);
